...
Model ResourceManager::loadModel(const std::string &path) {
  ...
  uint32_t start = geometries.size();
  uint32_t count = gltfMesh.primitives.size();
  geometries.resize(start + count);
  std::vector<Material> materials(count);
  for (int i = 0; i < count; i++) {
    auto &currentGeometry = geometries[start + i];
    ...
    if (material.normalTexture.index > -1) {
      addTexture(createTexture(model, material.normalTexture.index));
      finalMaterial.normalTexture = textures.size() - 1;
    }
    ...
    if (pbrMetallicRoughness.metallicRoughnessTexture.index > -1) {
      addTexture(createTexture(
          model, pbrMetallicRoughness.metallicRoughnessTexture.index));
      finalMaterial.metallicRoughnessTexture = textures.size() - 1;
    }
...
uint32_t ResourceManager::uploadGeometry(uint32_t i) {
  auto &geometry = geometries[i];
  gpuGeometries.push_back(uploadGeometryToGPU(geometry));
  return gpuGeometries.size() - 1;
}
